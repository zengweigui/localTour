...
$(document).ready(function() {
    ...
    $('#product-bg-input').on('change', function() {
        ...
        reader.onload = function(e) {
            ...
            img.src = this.result;
            img.onload = function() {
                $(".product-bg").attr("src", img.src);
            };
            that.siblings(".cancel-picture").show();
        };
    });
    ...
    $(".save-draft").click(function() {
    	var productBg = $(".product-bg").attr("src"), // 产品背景图
        ...
        $.ajax({
            url: "ajax/guideUploadProductAjax.jsp",
            type: "post",
            async: false,
            data: {
                "gphone": guidephone,
            ...
            success: function(data) {
                var data = $.trim(data);
                if (data == "success") {
                    window.location = "guideDraftsBox.jsp?gphone=" + guidephone;
                } else if (data == "failed") {
                    var txt = "保存草稿箱失败";
    ...
    function getAllTripJSON() {
        ...
        for (var i = 1; i <= tripItemSum; i++) {
            ...
            // 单个行程的所有图片的集合
            tripImgsObject = $("#trip-imgs-box-" + i + " img");
            console.log(tripImgsObject);

            // 循环获取单个行程中的所有图片的src并存入tripImgsArr数组中
